...
const requiredTools = ['git', 'pnpm'];

const motd = (cmd: string, printCmd: boolean) => {
  const commandSection = printCmd
...
async function warnIfOutdated(repo, clonedRepoDir, version) {
  if (repo === CIA_URL) {
    ...
    if (semver.lt(rootPkg.version, version)) {
      console.warn(
        c.yellow(
          `⚠️ You are using create-interchain-app version ${c.red(
            rootPkg.version
          )}, but version ${c.green(version)} is available. Run "${c.cyan(
            'cia upgrade'
          )}" or "${c.cyan(
            'npm install -g create-interchain-app@latest'
          )}" to upgrade.`
        )
      );
    }
  ...
  return async (argv) => {
    ...
    Proprietary and confidential`;
        ...
        if (hasResults) {
          ...
          content = content.replace(
            ...
            `${prefix}${results.__MODULENAME__}`
          );
        }
      }

      // Construct the file path
      const relativeFilePath = templateFile.split(
        toPosixPath(join(folderName, template) + '/')
      )[1];
    ...
    const closestPkgJson = []
      ...
      .reduce((shortest, current) => {
        return current.split(posixPath.sep).length <
          shortest.split(posixPath.sep).length
          ? current
          : shortest;
    ...
    if (closestPkgJson === fakeLongPath) {
      ...
      if (pkg.scripts) {
        ['locks:remove', 'locks:create', 'locks'].forEach(
      ...
      // Change to package directory and run pnpm if necessary
